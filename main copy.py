...
# # for ranking
# from ranking import rank_normalised,TrueRanker
# # for sord
# from soft_ordinal_loss import sord_loss

# import things for multi-head classification
...
def main():
    if args.dataset == 'imdb_wiki':
        max_age = 120
        global_class_indice = np.linspace(1,max_age,max_age)-0.5 # 0-120, so 121 classes in total
        global_class2count = np.linspace(0,max_age,max_age+1) # 121 classes in total
    elif args.dataset == 'agedb':
        max_age = 100
        global_class_indice = np.linspace(1+1,max_age,max_age)-0.5
        global_class2count = np.linspace(0+1,max_age,max_age+1) # 120 class in total
    ...
    global_count_dict = {x: 0 for x in range(max_age+1)} # the dict is the age:number of samples
    labels = df_train['age'].values # this is to get thelabel space
...
if __name__ == '__main__':
    # args.data_dir =  r'../hcnetv20729_agecodes/imdbwiki_data'
    # args.resume = r'./models/cls/ckpt.best.pth.tar'
    # main()
    # args.resume = r'./models/cls+lds/ckpt.best.pth.tar'
    # main()
    # args.resume = r'./models/cls+lds+r/ckpt.best.pth.tar'
    # main()
    main()
